...
bool verify_proof(r1cs_gg_ppzksnark_verification_key<ppT> verification_key, r1cs_primary_input<FieldT> primary_input,
        r1cs_gg_ppzksnark_proof<default_r1cs_gg_ppzksnark_pp> proof) {
    return r1cs_gg_ppzksnark_verifier_strong_IC<default_r1cs_gg_ppzksnark_pp>(verification_key, primary_input, proof);
}

r1cs_gg_ppzksnark_keypair<default_r1cs_gg_ppzksnark_pp> setup_gadget(protoboard<FieldT> &pb, block_variable<FieldT> *&inp, digest_variable<FieldT> *&out, sha256_two_to_one_hash_gadget<FieldT> *&g) {
...
int one_input_hash_gadget(int num_iterations) {
    ...
    r1cs_gg_ppzksnark_keypair<default_r1cs_gg_ppzksnark_pp> keypair = setup_gadget(pb, input, output, f);
    ...
    while (i < num_iterations) {
        ...
        const r1cs_gg_ppzksnark_proof<default_r1cs_gg_ppzksnark_pp> proof = r1cs_gg_ppzksnark_prover<default_r1cs_gg_ppzksnark_pp>(
